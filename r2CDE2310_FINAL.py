...
class AutoPilot(Node):
    def __init__(self):
        ...
        self.escdir = None # "LEFT" or "RIGHT" when escape direction is locked
        ...
        self.dock_target_dist = 0.30        # stop this far from marker
        ...
        self.dock_max_w = 0.10              # max angular speed while docking
        self.dock_lost_timeout = 0.75        # seconds before aborting if lost
        ...
        self.search_spin_speed = -0.05       # slow for camera ArUco detection
    ...
    def marker_callback(self, msg):
        ...
        if abs(alpha_check) > math.radians(10):
            self.get_logger().warn(
                f"Detected marker {chosen_id} at {min_distance:.2f}m but α={math.degrees(alpha_check):+.1f}° is too wide for docking. Ignoring.")
            self.marker_visible = False
            return
        self.last_seen = self.get_clock().now()
    ...
    def search_marker_logic(self):
        ...
        twist.linear.x = 0.0
        if self.escdir == "LEFT":
            twist.angular.z = self.search_spin_speed #search clockwise
        else:
            twist.angular.z = -self.search_spin_speed #search counter-clockwise
        self.publisher_.publish(twist)
