...
class AutoPilot(Node):
    def __init__(self):
        ...
        # ── Docking — 3-Phase Variables ──
        self.dock_staging_point = None      # MapNode in map frame
        self.dock_heading = None            # target yaw at staging point
        self.dock_marker_map = None         # (mx, my) marker position in map
        self.dock_plan_retries = 0          # retry counter for staging nav
        self.marker_x = None                # latest camera-frame X
        self.marker_z = None                # latest camera-frame Z
    ...
    def marker_callback(self, msg):
        """Detect ArUco markers.  On first detection during exploration,
        compute a staging point 0.4 m perpendicular in front of the marker
        in the map frame and reroute the path planner there."""
        ...
        if self.state in ['PLANNING', 'DRIVING', 'ALIGNING', 'SEARCHING']:
            self.get_logger().warn(
                f'Target Station ({chosen_id}) Detected at {min_distance:.2f}m! '
                f'Computing staging point...')
            self.stopbot()
            self.path = []
            self.goal = None
            self.search_last_yaw = None
            ...
            try:
                robot_x, robot_y, robot_yaw = self.get_orientation()
            except Exception as e:
                self.get_logger().error(f'Cannot get robot pose for staging: {e}')
                return

            # ── Transform marker position to map frame ──
            # Camera optical frame: X=right, Y=down, Z=forward
            cam_x = self.marker_x   # right offset in camera
            cam_z = self.marker_z   # forward offset in camera
            marker_map_x = (robot_x
                            + cam_z * math.cos(robot_yaw)
                            + cam_x * math.sin(robot_yaw))
            marker_map_y = (robot_y
                            + cam_z * math.sin(robot_yaw)
                            - cam_x * math.cos(robot_yaw))

            # ── Get marker normal from orientation quaternion ──
            qx = target_pose.orientation.x
            qy = target_pose.orientation.y
            qz = target_pose.orientation.z
            qw = target_pose.orientation.w

            # Marker Z-axis [0,0,1] rotated by quaternion → normal in camera frame
            ncx, ncy, ncz = self._quat_rotate_vector(qx, qy, qz, qw,
                                                       0.0, 0.0, 1.0)

            # Transform normal from camera frame to map frame (2-D projection)
            normal_map_x = ncz * math.cos(robot_yaw) + ncx * math.sin(robot_yaw)
            normal_map_y = ncz * math.sin(robot_yaw) - ncx * math.cos(robot_yaw)

            norm_len = math.sqrt(normal_map_x**2 + normal_map_y**2)
            if norm_len > 0.01:
                normal_map_x /= norm_len
                normal_map_y /= norm_len
            else:
                # Fallback: assume marker faces robot
                dx = robot_x - marker_map_x
                dy = robot_y - marker_map_y
                d = math.sqrt(dx**2 + dy**2)
                if d > 0.01:
                    normal_map_x = dx / d
                    normal_map_y = dy / d
                else:
                    normal_map_x = -math.cos(robot_yaw)
                    normal_map_y = -math.sin(robot_yaw)

            # Safety: make sure normal points toward robot (not into wall)
            to_robot_x = robot_x - marker_map_x
            to_robot_y = robot_y - marker_map_y
            dot = normal_map_x * to_robot_x + normal_map_y * to_robot_y
            if dot < 0:
                normal_map_x = -normal_map_x
                normal_map_y = -normal_map_y

            # ── Staging point 0.4 m in front of marker ──
            staging_x = marker_map_x + 0.4 * normal_map_x
            staging_y = marker_map_y + 0.4 * normal_map_y

            # Heading at staging point: face the marker
            dock_heading = math.atan2(marker_map_y - staging_y,
                                      marker_map_x - staging_x)

            self.dock_staging_point = MapNode(staging_x, staging_y)
            self.dock_heading = dock_heading
            self.dock_marker_map = (marker_map_x, marker_map_y)

            # Check if already close to staging point → skip nav
            dist_to_staging = math.sqrt(
                (staging_x - robot_x)**2 + (staging_y - robot_y)**2)

            if dist_to_staging < 0.20:
                self.get_logger().info(
                    'Already at staging point. Rotating to face marker...')
                self.state = 'DOCK_ROTATE'
            else:
                self.goal = self.dock_staging_point
                self.state = 'DOCK_NAV'

            self.get_logger().info(
                f'Marker map pos: ({marker_map_x:.2f}, {marker_map_y:.2f})  '
                f'Staging: ({staging_x:.2f}, {staging_y:.2f})  '
                f'Heading: {math.degrees(dock_heading):.1f}°')
    ...
    def mover(self, allow_state_change=True):
        ...
        if cmd_vel is None:
            ...
            if allow_state_change:
                ...
                # During DOCK_NAV, just rotate in place toward next waypoint
        ...
        if left_min < SIDE_NUDGE_DIST or right_min < SIDE_NUDGE_DIST:
            # Proportional: steer toward the side with MORE room
    ...
    def dock_nav_logic(self):
        """Phase 1: Navigate to the staging point using the path planner.
        Uses a progressive approach — if the exact staging point is
        unreachable (on a wall cell in the coarse grid), navigate to the
        nearest reachable cell, then replan from that closer position."""
        try:
            cur_x, cur_y, _ = self.get_orientation()
        except Exception:
            return

        dist = math.sqrt((self.dock_staging_point.x - cur_x)**2
                         + (self.dock_staging_point.y - cur_y)**2)

        # ── Close enough → proceed to rotation ──
        if dist <= 0.20:
            self.stopbot()
            self.path = []
            self.dock_plan_retries = 0
            self.state = 'DOCK_ROTATE'
            self.get_logger().info(
                f'Reached staging point ({dist:.2f}m). Rotating to face marker…')
            return

        # ── Need a path? Plan one. ──
        if not self.path or len(self.path) == 0:
            self.path = self.planroute(goal=self.dock_staging_point,
                                       allow_unknown=True)

            if not self.path:
                self.dock_plan_retries += 1

                # Reasonably close — just proceed to rotate
                if dist <= 0.35:
                    self.get_logger().warn(
                        f'Cannot plan closer (at {dist:.2f}m). '
                        f'Close enough — proceeding to DOCK_ROTATE.')
                    self.stopbot()
                    self.dock_plan_retries = 0
                    self.state = 'DOCK_ROTATE'
                    return

                # Too many retries — accept current position
                if self.dock_plan_retries >= 5:
                    self.get_logger().warn(
                        f'Staging unreachable after {self.dock_plan_retries} '
                        f'attempts (at {dist:.2f}m). Proceeding to DOCK_ROTATE.')
                    self.stopbot()
                    self.dock_plan_retries = 0
                    self.state = 'DOCK_ROTATE'
                    return
                ...
                self.get_logger().warn(
                    f'Plan to staging failed (attempt '
                    f'{self.dock_plan_retries}, {dist:.2f}m away). Retrying…')
                return
            else:
                # Plan succeeded — reset retry counter
                self.dock_plan_retries = 0

        # ── Follow the path ──
        self.mover(allow_state_change=False)

        # If mover consumed the entire path but we're still far,
        # clear it so next tick triggers a replan from our new position
        if (not self.path or len(self.path) == 0) and dist > 0.20:
            self.get_logger().info(
                f'Path exhausted but {dist:.2f}m from staging. '
                f'Will replan from closer position…')

    def dock_rotate_logic(self):
        """Phase 2: Rotate in place until facing the marker."""
        try:
            _, _, cur_yaw = self.get_orientation()
        except Exception:
            return

        angle_diff = self.dock_heading - cur_yaw
        angle_diff = (angle_diff + math.pi) % (2 * math.pi) - math.pi

        if abs(angle_diff) < 0.08:      # ~4.6°
            self.stopbot()
            self.state = 'DOCK_STRAIGHT'
            self.get_logger().info(
                '✓ Aligned to marker.  Driving straight to dock…')
            return
        ...
        max_w = 0.15
        w = angle_diff * kp
        twist.angular.z = max(-max_w, min(max_w, w))
        self.publisher_.publish(twist)

    def dock_straight_logic(self):
        """Phase 3: Drive straight forward until 0.20 m from the wall."""
        # Use narrow center cone (±10°) for precise distance to dock wall
        if len(self.laser_range) > 0:
            total_points = len(self.laser_range)
            ppd = total_points / 360.0
            center_half = max(1, int(10 * ppd))
            center_right = self.laser_range[-center_half:]
            center_left = self.laser_range[:center_half]
            center = np.concatenate((center_right, center_left))
            center_min = (np.nanmin(center)
                          if len(center) > 0
                          and not np.isnan(center).all()
                          else float('inf'))
        else:
        ...
        if center_min <= 0.20:
            ...
            self.state = 'STATION'
            self.get_logger().info(
                f'✓ Docked!  Front distance: {center_min:.2f}m')
            return

        cmd = Twist()
        cmd.linear.x = 0.08        # slow careful approach
        cmd.angular.z = 0.0

        # Minor angular correction if marker is still visible
        if self.marker_visible and self.marker_x is not None:
            alpha = math.atan2(self.marker_x, self.marker_z)
            correction = -0.3 * alpha
            cmd.angular.z = max(-0.15, min(0.15, correction))
    ...
    def controller(self):
        ...
        # ── Recovery & Escape ──
            ...
            if not self.checkObstacles():
                if elapsed < self.escape_duration:
                    ...
                    if self.pre_recovery_state in [
                            'DOCK_NAV', 'DOCK_ROTATE', 'DOCK_STRAIGHT']:
                        self.get_logger().info(
                            'Was docking. Resuming nav to staging point…')
                        self.path = []      # replan
                        self.state = 'DOCK_NAV'
                    else:
                        self.get_logger().info(
        ...
        # ── Docking — 3 Phases ──
        elif self.state == 'DOCK_NAV':
            self.dock_nav_logic()

        elif self.state == 'DOCK_ROTATE':
            self.dock_rotate_logic()

        elif self.state == 'DOCK_STRAIGHT':
            self.dock_straight_logic()
        ...
        # ── Station & Mission ──
            ...
            if self.responseHeard:
                ...
                self.responseHeard = False
                # Clear docking state
                self.dock_staging_point = None
                self.dock_heading = None
                self.dock_marker_map = None
